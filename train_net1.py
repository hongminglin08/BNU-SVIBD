...
def train_net(cfg):
    ...
    train_list={'volleyball':train_volleyball, 'collective':train_collective, 'interactiveBehaviordataset':train_collective}
    test_list={'volleyball':test_volleyball, 'collective':test_collective, 'interactiveBehaviordataset':test_collective}
    ...
    for epoch in range(start_epoch, start_epoch+cfg.max_epoch):
        ...
        if epoch % cfg.test_interval_epoch == 0:
            ...
            print_log(cfg.log_path, 
                      'Best group activity accuracy: %.2f%% and action accuracy: %.2f%% at epoch #%d.'%(best_result['activities_acc'], best_result['actions_acc'], best_result['epoch']))
            ...
            if cfg.training_stage==2:
                ...
                filepath=cfg.result_path+'/stage%d_epoch%d_%.2f%%_%.2f%%.pth'%(cfg.training_stage,epoch,test_info['activities_acc'],test_info['actions_acc'])
...
def train_collective(data_loader, model, device, optimizer, epoch, cfg):
    ...
    for batch_data in data_loader:
        ...
        actions_scores,activities_scores,boxes_states=model((batch_data[0],batch_data[1],batch_data[4]))
        #t = t + 1
        ...
        # Predict actions
        if cfg.training_stage==1: 
            actions_loss=my_cross_entropy(actions_scores,actions_in,reduction="sum")#
        else:
