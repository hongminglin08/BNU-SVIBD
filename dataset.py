    ...
    if cfg.dataset_name=='volleyball':
        ...
        training_set=CollectiveDataset(train_anns,train_frames,
                                      cfg.data_path,cfg.image_size,cfg.out_size,
                                       num_boxes=cfg.num_boxes,num_frames=cfg.num_frames,is_training=True,is_finetune=(cfg.training_stage==1))

        validation_set=CollectiveDataset(test_anns,test_frames,
                                      cfg.data_path,cfg.image_size,cfg.out_size,
                                         num_boxes=cfg.num_boxes,num_frames=cfg.num_frames,is_training=False,is_finetune=(cfg.training_stage==1))
        
    elif cfg.dataset_name=='interactiveBehaviordataset':
        train_anns=interaction_read_dataset(cfg.data_path, cfg.train_seqs)
        train_frames=interaction_all_frames(train_anns)

        test_anns=interaction_read_dataset(cfg.data_path, cfg.test_seqs)
        test_frames=interaction_all_frames(test_anns)

        training_set=InteractionDataset(train_anns,train_frames,
                                      cfg.data_path,cfg.image_size,cfg.out_size,
                                       num_boxes=cfg.num_boxes,num_frames=cfg.num_frames,is_training=True,is_finetune=(cfg.training_stage==1))

        validation_set=InteractionDataset(test_anns,test_frames,
                                      cfg.data_path,cfg.image_size,cfg.out_size,
                                         num_boxes=cfg.num_boxes,num_frames=cfg.num_frames,is_training=False,is_finetune=(cfg.training_stage==1))
